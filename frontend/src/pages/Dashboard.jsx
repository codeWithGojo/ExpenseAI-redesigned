...
export default function Dashboard({ onLogout }) {
  ...
  return <div className="min-h-screen bg-[#f7f8fa] text-slate-900">
    <aside className="fixed inset-y-0 left-0 hidden w-60 border-r border-slate-200 bg-white p-5 lg:flex lg:flex-col">
      ...
      <nav className="space-y-1">{[["overview","Overview"],["transactions","Transactions"],["goals","Savings goals"],["coach","AI coach"]].map(([id,label]) => <button key={id} onClick={() => setTab(id)} className={`w-full rounded-xl px-4 py-3 text-left text-sm font-semibold ${tab === id ? "bg-blue-50 text-blue-700" : "text-slate-500 hover:bg-slate-50"}`}>{label}</button>)}</nav>
    ...
    <main className="lg:ml-60">
      <header className="flex h-20 items-center justify-between border-b border-slate-200 bg-white px-5 sm:px-8"><div><h1 className="text-lg font-bold">Your money, clearly.</h1><p className="text-xs text-slate-500">Track progress and make confident decisions.</p></div><div className="grid h-10 w-10 place-items-center rounded-full bg-blue-100 text-sm font-bold text-blue-700">FI</div></header>
      <div className="mx-auto max-w-7xl space-y-5 p-5 sm:p-8">
        ...
        {loading ? <div className="grid min-h-64 place-items-center text-sm text-slate-500">Loading your finances…</div> : <>
          ...
          <section className="grid gap-5 xl:grid-cols-[1.3fr_.7fr]">
            ...
            <article className="rounded-2xl border border-slate-200 bg-white p-5"><h3 className="font-bold">AI insight</h3><p className="mt-1 text-xs text-slate-500">Based on your current records</p><div className="mt-5 rounded-xl bg-violet-50 p-4 text-sm leading-6 text-violet-950">{data.insights?.tips?.[0] || "Add some data to unlock personalised insights."}</div><form onSubmit={askAI} className="mt-4 flex gap-2"><input className={field} value={chat} onChange={e => setChat(e.target.value)} placeholder="Ask about spending…"/><button className={button}>Ask</button></form>{reply && <p className="mt-3 rounded-xl bg-slate-50 p-3 text-sm text-slate-600">{reply}</p>}</article>
          </section>
          <section className="grid gap-5 xl:grid-cols-3">
            <FormCard title="Add expense" onSubmit={e => { e.preventDefault(); submit("expenses", expense, () => setExpense({amount:"",category:"Food & dining",note:""})); }} button={button}><input className={field} type="number" min="0.01" step="0.01" required placeholder="Amount" value={expense.amount} onChange={e=>setExpense({...expense,amount:e.target.value})}/><select className={field} value={expense.category} onChange={e=>setExpense({...expense,category:e.target.value})}><option>Food & dining</option><option>Transport</option><option>Utilities</option><option>Shopping</option><option>Other</option></select><input className={field} placeholder="Note (optional)" value={expense.note} onChange={e=>setExpense({...expense,note:e.target.value})}/></FormCard>
            <FormCard title="Add income" onSubmit={e => { e.preventDefault(); submit("income", income, () => setIncome({amount:"",source:""})); }} button={button}><input className={field} type="number" min="0.01" step="0.01" required placeholder="Amount" value={income.amount} onChange={e=>setIncome({...income,amount:e.target.value})}/><input className={field} required placeholder="Source" value={income.source} onChange={e=>setIncome({...income,source:e.target.value})}/></FormCard>
            <FormCard title="New savings goal" onSubmit={e => { e.preventDefault(); submit("goals", goal, () => setGoal({title:"",targetAmount:"",deadline:""})); }} button={button}><input className={field} required placeholder="Goal name" value={goal.title} onChange={e=>setGoal({...goal,title:e.target.value})}/><input className={field} type="number" min="0.01" step="0.01" required placeholder="Target amount" value={goal.targetAmount} onChange={e=>setGoal({...goal,targetAmount:e.target.value})}/><input className={field} type="date" value={goal.deadline} onChange={e=>setGoal({...goal,deadline:e.target.value})}/></FormCard>
          </section>
          <section className="grid gap-5 lg:grid-cols-2"><List title="Recent expenses" items={data.expenses} empty="No expenses recorded." render={item => <Row key={item.id} title={item.note || item.category} meta={`${item.category} · ${new Date(item.date).toLocaleDateString()}`} amount={`−${cash(item.amount)}`} onDelete={()=>remove("expenses",item.id)}/>} /><List title="Savings goals" items={data.goals} empty="No savings goals yet." render={item => <div key={item.id} className="border-t border-slate-100 py-4 first:border-0"><div className="flex justify-between text-sm"><strong>{item.title}</strong><span>{cash(item.savedAmount)} / {cash(item.targetAmount)}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-blue-600" style={{width:`${Math.min(100,(item.savedAmount/item.targetAmount)*100)}%`}}/></div></div>} /></section>
        </>}
      </div>
    </main>
  </div>;
